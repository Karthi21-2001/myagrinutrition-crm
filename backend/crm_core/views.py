...
def export_visits_to_excel(request):
    ...
    for v in all_visits:
        ...
        for p in product_loop_list:
            ...
            ws_data.cell(row=current_row, column=2, value=f.next_visit_date if f else "")
            ws_data.cell(row=current_row, column=3, value=nvd.strftime("%Y-%m-%d") if nvd else "")
            ws_data.cell(row=current_row, column=4, value=v.executive.username if v and v.executive else "")
            ws_data.cell(row=current_row, column=5, value=f.farm_name if f else "")
            ws_data.cell(row=current_row, column=6, value=f.owner_name if f else "")
            ws_data.cell(row=current_row, column=7, value=f.contact_number if f else "")
            ws_data.cell(row=current_row, column=8, value=f.business_type if f else "")
            ws_data.cell(row=current_row, column=9, value=f.sub_segment if (f and f.sub_segment) else "")
            ws_data.cell(row=current_row, column=10, value=f.state if f else "")
            ws_data.cell(row=current_row, column=11, value=normalize_district(f.district) if f else "")
            ws_data.cell(row=current_row, column=12, value=f.area if f else "")

            ws_data.cell(row=current_row, column=13, value=v.farm_problem if (v and v.farm_problem) else "None reported")

            ws_data.cell(row=current_row, column=14, value=getattr(f, 'chicks_count', 0) if f else 0)
            ws_data.cell(row=current_row, column=15, value=getattr(f, 'grower_count', 0) if f else 0)
            ws_data.cell(row=current_row, column=16, value=getattr(f, 'layer_count', 0) if f else 0)
            ws_data.cell(row=current_row, column=17, value=getattr(f, 'culling_bird_count', 0) if f else 0)

            ws_data.cell(row=current_row, column=18, value=p.product_name if p else "General Consult")
            ws_data.cell(row=current_row, column=19, value=p.sale_quantity if p else 0)
            ws_data.cell(row=current_row, column=20, value=float(p.primary_price) if p else 0.0)
            ws_data.cell(row=current_row, column=21, value=float(p.revenue_generated) if p else 0.0)

            ws_data.cell(row=current_row, column=22, value=p.potential_quantity if p else 0)
            ws_data.cell(row=current_row, column=23, value=p.target_quantity if p else 0)
            ws_data.cell(row=current_row, column=24, value=p.unit_type if p else "N/A")
            ws_data.cell(row=current_row, column=25, value=p.process_status if p else "N/A")
            ws_data.cell(row=current_row, column=26, value=f"{p.conversion_percentage}%" if p else "0%")
