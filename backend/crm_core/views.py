...
def get_dashboard_context(request):
    ...
    try:
        ...
        total_rev = float(
            product_qs.aggregate(total=Coalesce(Sum("revenue_generated", output_field=FloatField()), 0.0))[
                "total"
            ]
        )
        vol_sold = int(
        ...
        month_wise_qs = list(
            ...
            .values("month")
            .annotate(revenue=Coalesce(Sum("revenue_generated", output_field=FloatField()), 0.0))
        ...
        exec_perf = (
            ...
            .annotate(
                revenue=Coalesce(Sum("revenue_generated", output_field=FloatField()), 0.0),
        ...
        monthly_sales = list(
            ...
            .annotate(
                ...
                total_revenue=Coalesce(Sum("revenue_generated", output_field=FloatField()), 0.0),
            )
            .filter(month__isnull=False)
            .order_by("-month")[:25]
        )
